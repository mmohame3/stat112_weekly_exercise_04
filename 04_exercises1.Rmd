...
> There are many starbucks in North America
...
> It changes the level of details. That is, if the zoom number is lower, the map gets blurry. If you go higher, the map is more detailed but will take more time to load.
...
For the first chunk:
Line 1 reads in the 2018 us census data.
...
Line 1 reads in the starbucks_us_by_state data
...
Trips_by_Station
#leaflet(data = Trips_by_Station) %>% #base plot
...
> Stations in the city center have a higher potential of casual clients.
...
covid19 %>% 
  ...
  scale_fill_viridis_c()
```
 > It looks like the States with more population have the highest cases.
...
```
  ...
  Question: Try to visualise the relationship between sus_prop and poverty
```{r}
# as.data.frame(mpls_all)
```
```{r}
pal3 <- colorNumeric("viridis", 
                     domain = mpls_all$sus_prop)
